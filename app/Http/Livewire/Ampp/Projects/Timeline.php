    ...
    public function render(): View
    {
        ...
        $activities = Activity::whereNotNull(['causer_type', 'subject_type'])
            ...
            ->filter(function ($value, $key){
                if ($value->subject == $this->project->withoutRelations()){
                    return true;
                }

                if ($value->subject_type == TimeRegistration::class && $value->subject?->project_id == $this->project->id){
                    return true;
                }

                if ($value->subject_type == Todo::class && $value->subject?->morphable == $this->project->withoutRelations()){
                    return true;
                }

                if ($value->subject_type == Note::class && $value->subject?->morphable == $this->project->withoutRelations()){
                    return true;
                }

                if ($value->subject_type == Email::class && $value->subject?->morphable == $this->project->withoutRelations()){
                    return true;
                }
