<x-layouts.ampp :title="__('Edit expense')" :breadcrumbs="Breadcrumbs::render('editExpense', $expense)">
    <div class="container">
        <x-ui.page-title>{{ __('Edit expense') }}</x-ui.page-title>

        <div class="card card-body">
            <x-forms.form action="{{ action(\App\Http\Controllers\Ampp\Expenses\UpdateExpenseController::class, $expense) }}" method="patch" has-files>
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <x-forms.label for="name">{{ __('Name (when empty, generated automatically)') }}</x-forms.label>
                        <x-forms.input name="name" :value="$expense->name" />
                    </div>
                ...
                <div class="row">
                    ...
                    >
                        ...
                        <x-forms.input name="invoice_date" :value="$expense->invoice_date" x-ref="input" required />
                    </div>

                    <div class="col-md-4 mb-3">
                        <x-forms.label for="invoice_number">{{ __('Invoice number') }}</x-forms.label>
                        <x-forms.input name="invoice_number" :value="$expense->invoice_number" />
                    </div>
                    ...
                    >
                        ...
                        <x-forms.input name="invoice_ogm" :value="$expense->invoice_ogm" x-ref="input" />
                    </div>
                </div>

                <div
                    x-data="{
                        amountExcludingVat: {{ json_encode(old('amount_excluding_vat', $expense->amount_excluding_vat)) }},
                        amountVat: {{ json_encode(old('amount_vat', $expense->amount_vat)) }},
                        amountVatPercentage: {{ json_encode(old('amount_vat_percentage', $expense->amount_vat_percentage)) }},
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <x-forms.select
                            ...
                            :values="$expense->various_transaction_category ? $expense->various_transaction_category->value : \App\Enums\VariousTransactionCategory::Other->value"
                            required
                        />
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <x-forms.select name="invoice_category_id" :options="$invoiceCategories" :values="$expense->invoice_category_id" />
                    </div>
                </div>

                <div class="mb-3">
                    <x-forms.label for="comment">{{ __('Comment') }}</x-forms.label>
                    <x-forms.quill name="comment" :value="$expense->comment" />
                </div>
                ...
                <div class="mb-3">
                    <div
                        x-data="{
                            pond: null,
                            file: {{ json_encode($expense->getFirstMedia() ? action(\App\Http\Controllers\Media\DownloadMediaController::class, $expense->getFirstMedia()): null) }},
                            name: {{ json_encode($expense->getFirstMedia()?->file_name) }}
                        }"
                        x-init="
                ...
                <div class="d-flex justify-content-end">
                    <x-forms.submit>{{ __('Edit expense') }}</x-forms.submit>
                </div>
            </x-forms.form>
        </div>
    </div>
