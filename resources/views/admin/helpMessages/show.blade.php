<x-layouts.ampp :title="$helpMessage->title" :breadcrumbs="Breadcrumbs::render('showHelpMessage', $helpMessage)">
    <div class="container">
        <div class="card card-body">
            <div class="row mb-5">
                <div class="col-xl-2">
                    <p class="fs-4 fw-light">{{ __('Help message') }}</p>
                </div>
                            ...
                            @foreach($helpMessage->getMedia('images') as $media)
                                <div class="col-lg-4">
                                    <a href="{{ action(\App\Http\Controllers\Media\DownloadMediaController::class, $media) }}">
                                        <img
                                            src="{{ action(\App\Http\Controllers\Media\ShowMediaController::class, $media) }}"
                                            alt="image" class="img-fluid rounded-3 shadow"
