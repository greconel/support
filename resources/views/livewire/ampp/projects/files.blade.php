        ...
        @foreach($project->getMedia('files') as $file)
            <tr>
                ...
                <td class="d-flex justify-content-end">
                    <div class="dropdown">
                        <a href="#" class="text-decoration-none link-secondary ps-4" type="button" id="fileDropdown_{{ $file->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                            <i class="fas fa-ellipsis-v"></i>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-start" aria-labelledby="fileDropdown_{{ $file->id }}">
                            <li><button class="dropdown-item" wire:click="download('{{ $file->id }}')">{{ __('Download') }}</button></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><button class="dropdown-item" wire:click="delete('{{ $file->id }}')">{{ __('Remove') }}</button></li>
                        </ul>
                    </div>
                </td>
            </tr>
